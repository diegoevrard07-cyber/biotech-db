...
def test_build_targets_long_and_short():
    rows = [
    ...
    targets = ps.build_targets(rows, equity=10_000, prices={"AAA": 10.0, "BBB": 20.0})
    assert targets["AAA"]["side"] == "long"
    assert targets["AAA"]["target_shares"] == 50.0
    assert targets["BBB"]["side"] == "short"
    assert targets["BBB"]["target_shares"] == 15.0
