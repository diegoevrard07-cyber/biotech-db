...
def test_build_targets_long_only_skips_shorts():
    rows = [
        {"ticker": "AAA", "weight": 0.05, "trade_type": "buy_the_rumor",
         "expected_date": date(2026, 8, 1), "company_id": 1, "catalyst_id": 10},
        {"ticker": "BBB", "weight": -0.03, "trade_type": "fade",
         "expected_date": date(2026, 9, 1), "company_id": 2, "catalyst_id": 20},
        {"ticker": "CCC", "weight": 0.04, "trade_type": "fade",  # stale fade label
         "expected_date": date(2026, 9, 1), "company_id": 3, "catalyst_id": 30},
    ]
    targets = ps.build_targets(rows, equity=10_000,
                               prices={"AAA": 10.0, "BBB": 20.0, "CCC": 10.0})
