...
def run(*, dry_run: bool = False) -> None:
    with get_connection() as conn:
        ...
        try:
            ...
            net_short_realized = sum(
                float(h["realized_pnl_usd"] or 0.0) for h in shorts
            )
            ...
            if open_shorts:
                print(f"  WARNING: {len(open_shorts)} short(s) still OPEN — their market "
                      "value is dropped; cover them first for an exact reversal.")

            # ---- recompute long-only performance snapshots ----
            cur.execute("SELECT snapshot_date, equity, realized_to_date FROM portfolio_performance "
            ...
            for snap_date, equity, realized_td in snaps:
                cur.execute(
                    ...
                    "AND exit_date <= %s", (snap_date,))
                long_realized = float(cur.fetchone()[0] or 0.0)
                cur.execute(
                    "SELECT COALESCE(SUM(realized_pnl_usd),0) FROM portfolio_holdings "
                    "WHERE side='short' AND exit_date <= %s", (snap_date,))
                ...
                snap_updates.append({
                    "date": snap_date, "old_equity": float(equity or 0.0),
            ...
            for s in snap_updates:
                print(f"    {s['date']}: equity ${s['old_equity']:,.0f}->${s['new_equity']:,.0f}  "
            ...
            payload = {
                ...
                "cash_before": cash, "cash_after": new_cash,
                "holdings": [{k: _jsonable(v) for k, v in h.items()} for h in shorts],
                "snapshots_before": [{"date": str(d), "equity": float(e or 0), "realized_to_date": float(r or 0)}
                                     for d, e, r in snaps],
            }
            ...
            cur.execute("CREATE TABLE IF NOT EXISTS shorts_removed_backup (LIKE portfolio_holdings)")
            cur.execute("ALTER TABLE shorts_removed_backup ADD COLUMN IF NOT EXISTS removed_at TIMESTAMPTZ DEFAULT NOW()")
