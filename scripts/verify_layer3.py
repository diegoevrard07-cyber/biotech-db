...
def verify_layer3() -> bool:
    ...
    with engine.connect() as conn:
        ...
        for r in rows:
            print(
                f"  {r[0]} | {r[1] or '-'} | {r[2] or '-'} | n={r[3]} rate={float(r[4]):.1%} tier={r[5]}"
            )
        ...
        print(
            "  - PDUFA/adcom rates use industry priors (source=industry_prior), not openFDA CRL data."
        )
